...
class OfficialCROGGPUConfigTest(unittest.TestCase):
    ...
    def test_official_training_hyperparameters_are_preserved(self):
        ...
        expected_lines = (
            ...
            r"^\s*sync_bn:\s*True\b",
            r"^\s*amp:\s*True\b",
            r"^\s*pin_memory:\s*True\s*$",
    ...
    def test_training_path_uses_cuda_and_has_no_npu_calls(self):
        paths = (
            ROOT / "train_crog.py",
            ROOT / "test_crog.py",
        ...
        forbidden = ("torch_npu", "utils.npu", "'hccl'", '"hccl"')
        for path in paths:
            source = path.read_text(encoding="utf-8")
        ...
        runtime = (ROOT / "utils" / "cuda.py").read_text(encoding="utf-8")
        self.assertIn('backend="nccl"', trainer)
        self.assertIn("torch.cuda.set_device", runtime)

    def test_single_gpu_evaluation_loads_ddp_checkpoints(self):
        source = (ROOT / "test_crog.py").read_text(encoding="utf-8")
        self.assertIn("set_device(args.gpu)", source)
    ...
    def test_sync_batchnorm_is_enabled_for_cuda_profiles(self):
        ...
        trainer = (ROOT / "train_crog.py").read_text(encoding="utf-8")
        self.assertIn("convert_sync_batchnorm", trainer)
    ...
    def test_launcher_derives_process_count_from_visible_gpus(self):
        launcher = (
            ROOT / "tools" / "train_gpu.sh"
        ).read_text(encoding="utf-8")
        trainer = (ROOT / "train_crog.py").read_text(encoding="utf-8")
        self.assertIn('CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"', launcher)
        self.assertIn('NPROC_PER_NODE="${#CUDA_DEVICE_LIST[@]}"', launcher)
    ...
    def test_amp_can_be_disabled_without_constructing_a_grad_scaler(self):
        runtime = (ROOT / "utils" / "cuda.py").read_text(encoding="utf-8")
        launcher = (
            ROOT / "tools" / "train_gpu.sh"
        ).read_text(encoding="utf-8")
    ...
    def test_adam_uses_cuda_foreach_when_enabled(self):
        trainer = (ROOT / "train_crog.py").read_text(encoding="utf-8")
        config = (
            ROOT / "config" / "OCID-VLG" / "etrg.yaml"
        ).read_text(encoding="utf-8")
        self.assertIn(
            'getattr(args, "optimizer_foreach", True)', trainer
        )
        ...
        self.assertRegex(
            config, r"(?m)^\s*optimizer_foreach:\s*True\s*$"
        )
    ...
    def test_each_training_run_uses_a_timestamped_output_directory(self):
        ...
        launchers = (
            ROOT / "tools" / "train_gpu.sh",
            ROOT / "tools" / "train_toolrgs_model_gpu.sh",
        )
        self.assertIn(
    ...
    def test_drogoff_inference_ablations_are_checkpoint_compatible(self):
        ...
        for name, (use_offset, mask_filter, resample) in expected.items():
            ...
            with self.subTest(config=name):
                ...
                expected_batch_size = 24 if name == "drogoff.yaml" else 32
                self.assertRegex(
                    source,
                    rf"(?m)^\s*batch_size:\s*{expected_batch_size}\b",
                )
    ...
    def test_drog_configs_match_the_requested_global_schedule(self):
        expected_optimization = {
            ...
            "drogoff.yaml": ("drogoff", 24, 32, "0.0001", 30),
        }
        for name, (
            architecture,
            batch_size,
